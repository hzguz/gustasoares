                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 lg:gap-12 xl:gap-32 items-center">
                    ...
                    <div className="order-2 xl:order-1">
                        <Reveal variant="fade-up">
                            <h2 className="font-syne font-bold text-2xl md:text-4xl lg:text-5xl mb-6 md:mb-8 leading-tight">
                            ...
                            <div className="font-manrope text-sm md:text-base lg:text-lg leading-relaxed whitespace-pre-line mb-8 md:mb-10 bg-gradient-to-br from-[#111111] to-[#ababab] bg-clip-text text-transparent">
                                {aboutPage.history.description}
                            </div>

                            {/* Highlights - 2 columns on mobile, 3 on md+ */}
                            <div className="relative grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-6 p-2 md:p-2 border border-black/[0.08] rounded-2xl md:rounded-[32px]">
                                {aboutPage.history.highlights.map((highlight, idx) => {
                                    const isFirst = idx === 0;
                                    return (
                                        <div
                                            key={idx}
                                            className={`relative z-10 flex flex-col gap-1 md:gap-3 p-3 md:p-8 rounded-xl md:rounded-3xl ${isFirst ? 'col-span-2 md:col-span-1 bg-gradient-to-br from-[#353535] to-[#111111]' : 'border-black/[0.04]'}`}
                                        >
                                            <span className={`font-syne font-bold text-xl md:text-2xl xl:text-4xl ${isFirst ? 'text-white' : 'text-textPrimary'}`}>
                                                {highlight.number}
                                            </span>
                                            <span className={`font-manrope text-[10px] md:text-xs uppercase tracking-wider ${isFirst ? 'text-white/80' : 'bg-gradient-to-r from-[#111111] to-[#ababab] bg-clip-text text-transparent'}`}>
                                                {highlight.label}
                                            </span>
                                        </div>
                                    );
                    ...
                    <div className="order-1 xl:order-2 relative">
                        <Reveal variant="fade-left" delay={200}>
                            <div className="relative aspect-square group">
                                {/* Main Image Container - Hover effects only on md+ */}
                                <div className="relative z-10 rounded-[2rem] overflow-hidden border border-black/[0.04] h-full shadow-2xl transition-transform duration-500 md:group-hover:-translate-y-1">
                                    <img
                                        src={aboutPage.history.image}
                                        alt="Gusta Soares"
                                        className="w-full h-full object-cover transition-transform duration-700 md:group-hover:scale-105 contrast-[1.1]"
                                    />
                                ...
                                {/* Tech Corners - Cor preta, 1px, aparece no hover - desktop only */}
